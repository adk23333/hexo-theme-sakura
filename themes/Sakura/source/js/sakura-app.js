...
function timeSeriesReload(flag) {
    if (flag === true) {
        ...
        (function () {
            ...
            const $arch_al_mon = $('#archives span.al_mon')
            $arch_al_mon.each(function () {
                const num = $(this).next().children('li').length;
                $(this).children('#post-num').text(num)
            })
            const $al_post_list = $('#archives ul.al_post_list'),
                $al_post_list_f = $('#archives ul.al_post_list:first');
            $al_post_list.hide(1, function () {
                $al_post_list_f.show()
            })
            $arch_al_mon.click(function () {
                $(this).next().slideToggle(400)
                return false
            })
            if (document.body.clientWidth > 860) {
